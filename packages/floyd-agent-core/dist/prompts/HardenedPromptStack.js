 ...
 */
export const DEFAULT_PROMPT_STACK = {
    identity: {
        name: 'FLOYD',
        version: '1.0.0',
        capabilities: [
 ...
 */
export function buildSystemPrompt(stack) {
    const sections = [];
    // Identity Layer
 ...
 */
export function createPromptStack(domainLanguages) {
    const stack = { ...DEFAULT_PROMPT_STACK };
    if (domainLanguages && domainLanguages.length > 0) {
        stack.domain = {};
        domainLanguages.forEach(lang => {
            const langLower = lang.toLowerCase();
            if (DOMAIN_PROMPTS[langLower]) {
                stack.domain[langLower] = DOMAIN_PROMPTS[langLower];
            }
        });
    }
    return stack;
}
//# sourceMappingURL=HardenedPromptStack.js.map