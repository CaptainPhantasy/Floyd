    ...
    for (const relativePath of DEFAULT_CONFIG_PATHS) {
        ...
        if (existsSync(fullPath)) {
            try {
                ...
                throw new Error(`Failed to load MCP config from ${fullPath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
 ...
 */
export function loadMCPConfigFromFile(filePath) {
