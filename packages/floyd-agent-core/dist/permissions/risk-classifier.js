 ...
 */
export function classifyRisk(toolName, arguments_ = {}) {
    const reasons = [];
    ...
    if (level === RiskLevel.LOW) {
        for (const pattern of LOW_RISK_PATTERNS) {
            if (pattern.test(toolName)) {
                reasons.push(`Tool matches low-risk (read-only) pattern: ${pattern.source}`);
                confidence = 0.9;
                break;
            }
        }
    ...
    for (const pattern of DANGEROUS_ARG_PATTERNS) {
        if (pattern.test(argsString)) {
            ...
            reasons.push(`Arguments contain potentially dangerous pattern: ${pattern.source}`);
    ...
    if (reasons.length === 0) {
        reasons.push('Tool behavior not explicitly categorized - using default assessment');
        confidence = 0.5;
    }
    return {
        level,
